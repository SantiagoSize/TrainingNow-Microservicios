...
- **TrainNow-Usuarios** (`src/main/java/com/tn/usuarios/security/SecurityConfig.java`): exigen rol `ADMIN` — `POST /api/users/admin-create`, `PATCH /api/users/*/ban|unban|suspend|unsuspend`, `POST`/`GET /api/audit-logs`, `GET /api/reports`, `PATCH /api/reports/*/resolve`. Todo lo demás (login, registro, listar, buscar) sigue abierto. 401 → `{"error":"Falta el token de autorización"}`, 403 → `{"error":"Operación permitida solo para administradores"}`.
